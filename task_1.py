...
def check_1(lst_obj):
    ...
    Сложность: O(1)
    """
    lst_to_set = set(lst_obj)  # O(len(lst_obj))O(len(lst_obj)) ~ O(1)
...
def check_3(lst_obj):
    ...
    Сложность: O(nlog(n))O(nlog(n)) + O(n)
    """
    lst_copy = list(lst_obj)                 # O(len(lst_obj)) ~ O(1)
    lst_copy.sort()                          # O(nlog(n))O(nlog(n))
